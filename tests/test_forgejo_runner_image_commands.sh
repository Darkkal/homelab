...
runner_vars="$repo_root/inventory/group_vars/forgejo_runner_hosts.yml"
...
rg -q -- '^      labels:$' "$config_template"
rg -q -- '^    daemon_reload: true$' "$handler"
rg -q -- 'name: Reload Forgejo runner user systemd' "$registration_task"
! rg -q -- '^Network=host$' "$quadlet_template"
! rg -q -- 'forgejo_runner_job_network: host' "$runner_vars"
rg -Fq -- 'host.containers.internal' "$runner_vars"
rg -Fq -- '  network: "{{ forgejo_runner_job_network }}"' "$config_template"
