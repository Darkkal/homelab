...
| **AdGuard Home** | `https://adguardhome.home` | `http://<host-ip>:8090` | Web UI / DNS (53) | LAN DNS server & ad blocker. Serves a wildcard `*.home` rewrite so any device pointed at `<host-ip>` (port 53) resolves every `*.home` hostname, with public queries forwarded to Quad9. Admin UI proxied via Caddy. | `service_hosts` |
...
- **Admin UI**: `https://adguardhome.home` (via Caddy) or `http://<host-ip>:8090` directly.
