...
- **DNS Hostname Resolution**: LAN clients resolve services using `*.home` hostnames served by the AdGuard Home DNS server on port `53`.
...
- **`service_hosts`**:
  ...
  - Deploys `caddy`, `sillytavern`, `open-webui`, `searxng`, `playwright`, `piclaw`, `forgejo`, `homepage`, `glances`, `uptrace` (with ClickHouse, PostgreSQL, Redis, and an OpenTelemetry Collector), `adguardhome` (LAN DNS server & ad blocker serving `*.home` rewrites so mDNS-less devices can resolve service hostnames).
...
1. **Host Name Resolution**:
   - The AdGuard Home DNS server answers every `*.home` hostname with the host's LAN IP, and forwards all other queries upstream to Quad9.
